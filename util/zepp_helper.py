...
UA = "MiFit6.14.0 (M2007J1SC; Android 12; Density/2.75)"
DEFAULT_BAND_HOST = "https://api-mifit-cn.huami.com"
...
def _ttl_from_object(obj):
    ...
    if not isinstance(obj, dict):
        return None
    stp = obj.get("stp")
    if isinstance(stp, (int, float)) and not isinstance(stp, bool):
        return max(0, int(stp))
    if isinstance(stp, dict):
        for key in ("ttl", "total", "step"):
            n = stp.get(key)
            try:
...
def _decode_base64_utf8(raw):
    normalized = str(raw or "").replace(" ", "").replace("-", "+").replace("_", "/")
    if not normalized:
        return ""
...
def parse_summary_steps(summary):
    if summary is None or summary == "":
        return None
    if isinstance(summary, bool):
        return None
    if isinstance(summary, (int, float)):
        return max(0, int(summary))
    if isinstance(summary, dict):
        return _ttl_from_object(summary)
    text = str(summary).strip()
    if not text:
        return None
    as_json = _try_json(text)
    if as_json is not None:
        from_json = _ttl_from_object(as_json)
        if from_json is not None:
            return from_json
    try:
        decoded = _decode_base64_utf8(text)
        nested = _try_json(decoded)
        if nested is not None:
            return _ttl_from_object(nested)
    except Exception:
        pass
    return None


def steps_from_band_data(body, date):
    rows = body.get("data") if isinstance(body, dict) else None
    if not isinstance(rows, list) or not rows:
        return 0
    wanted = str(date or "")
    row = next(
        (item for item in rows if str(item.get("date") or item.get("date_time") or "") == wanted),
        rows[0],
    )
    steps = parse_summary_steps(row.get("summary") if isinstance(row, dict) else None)
    if steps is None:
        raise ValueError("无法解析华米当日步数")
    return steps


def get_band_summary(app_token, user_id, date):
    attempts = (
        {"device_type": "android_phone"},
        {"device_type": "0", "byteLength": "8"},
    )
    last_error = Exception("读取步数异常")
    t = get_time()
    for extra in attempts:
        ...
        try:
            resp = requests.get(
                f"{DEFAULT_BAND_HOST}/v1/data/band_data.json",
                params=params,
                headers=headers,
                timeout=12,
            )
        ...
        if resp.status_code != 200:
            ...
            if isinstance(parsed, dict) and parsed.get("message"):
                hint = str(parsed.get("message"))[:80]
            last_error = Exception(f"读取步数异常：{resp.status_code}" + (f" {hint}" if hint else ""))
            continue
        body = _try_json(resp.text)
        if not body:
            last_error = Exception("读取步数失败")
            continue
        ...
        try:
            ...
            code = None
        if message and message != "success" and code != 1:
            last_error = Exception(message or "读取步数失败")
            continue
        return body
    raise last_error


def fetch_today_steps(app_token, user_id, date=None):
    if not app_token or not user_id:
        raise ValueError("缺少登录票，无法读取华米当天步数")
    if date is None:
        date = get_beijing_time().strftime("%Y-%m-%d")
    body = get_band_summary(app_token, user_id, date)
    return steps_from_band_data(body, date)
